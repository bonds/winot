...
main = do
    ...
    if vpnEnabled world' then do
        ...
        let mask = configString "vpn_private_netmask" world'
        if B.isJust cpip && B.isJust spip && B.isJust mask && B.isJust vif then do
...
mainLoop world = do
    ...
    L.debugM logPrefix $ T.unpack $ "lastLoop: " `T.append` T.pack (show (head (loopTimes world')))
